...
const validateBasicGameStructure = (data) => {
  ...
  if (!data.name || typeof data.name !== 'string') {
    return { valid: false, error: 'Game name is required and must be a string' };
  }
...
const validateAnswer = (answer, questionIndex, answerIndex) => {
  if (!answer.text || typeof answer.text !== 'string') {
    return { valid: false, error: `Question ${questionIndex + 1}, Answer ${answerIndex + 1}: Text is required and must be a string` };
  }
  
  if (answer.isCorrect === undefined || typeof answer.isCorrect !== 'boolean') {
    return { valid: false, error: `Question ${questionIndex + 1}, Answer ${answerIndex + 1}: isCorrect is required and must be a boolean` };
  }
...
const validateQuestion = (question, index) => {
  if (!question.text || typeof question.text !== 'string') {
    return { valid: false, error: `Question ${index + 1}: Text is required and must be a string` };
  }
  
  if (!question.type || !['single', 'multiple', 'judgement'].includes(question.type)) {
  ...
  if (!question.points || typeof question.points !== 'number' || question.points <= 0) {
    return { valid: false, error: `Question ${index + 1}: Points must be a positive number` };
  }
  
  if (!Array.isArray(question.answers) || question.answers.length === 0) {
    return { valid: false, error: `Question ${index + 1}: Answers must be a non-empty array` };
  }
  ...
  if (question.imageUrl && typeof question.imageUrl !== 'string') {
    return { valid: false, error: `Question ${index + 1}: Image URL must be a string` };
  }
  
  if (question.videoUrl && typeof question.videoUrl !== 'string') {
    return { valid: false, error: `Question ${index + 1}: Video URL must be a string` };
  }
